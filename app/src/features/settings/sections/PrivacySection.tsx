...
const PRIVACY_POLICY_URL =
	"https://loris-galland.github.io/ERIS-SOSMap/privacy-policy";
		...
		<section>
			...
			<div className="bg-eris-surface-alt/40 border border-eris-border/50 rounded-3xl overflow-hidden">
				...
				<div className="flex items-center justify-between p-4 border-b border-eris-border/30">
					<div>
						...
						<p className="text-eris-text-subtle text-[11px]">
							{t(
								"settings.shareLocationDesc",
								"Allow rescue teams to find you",
							)}
						</p>
					</div>
					<Switch
				...
				<div className="flex items-center justify-between p-4 border-b border-eris-border/30">
					<div>
						<p className="text-eris-text text-sm font-medium">
							{t("settings.analytics", "Anonymous Analytics")}
						</p>
						<p className="text-eris-text-subtle text-[11px]">
					...
					<div className="bg-eris-surface/30 border-t border-eris-border/30 px-4 py-3 flex flex-col gap-3 animate-in fade-in slide-in-from-top-2 duration-300">
						...
						{(
							[
								{
									key: "crash",
									label: t("settings.sosTypeCrash", "Vehicle Crash Detection"),
								},
								{
									key: "fall",
									label: t("settings.sosTypeFall", "Fall Detection"),
								},
								{
									key: "discrete",
									label: t(
										"settings.sosTypeDiscrete",
										"Discrete SOS (Stealth)",
									),
								},
								{
									key: "shake",
									label: t("settings.sosTypeShake", "Shake-to-SOS"),
								},
								{
									key: "manual",
									label: t("settings.sosTypeManual", "Manual Button Press"),
								},
							] as const
						).map((item) => (
							<div key={item.key} className="flex items-center justify-between">
								<span className="text-eris-text text-xs font-medium">
									{item.label}
								</span>
								<Switch
									active={recordingPrefs[item.key]}
			...
			{showConsent && (
				<div className="mt-3 bg-eris-surface border border-eris-primary/40 rounded-2xl p-4 flex flex-col gap-3 animate-in fade-in slide-in-from-top-2 duration-300">
					<div className="flex items-center gap-2">
						<span className="material-symbols-outlined text-eris-primary text-xl">
							policy
						</span>
						<p className="text-eris-text font-bold text-sm">
							{t(
								"settings.audioConsentTitle",
								"Before enabling audio recording",
							)}
						</p>
					</div>
					<ul className="text-eris-text-muted text-[11px] leading-relaxed space-y-1 pl-1">
						...
						<li>
							•{" "}
							{t(
								"settings.audioConsent2",
								"Recordings are stored securely and deleted after 30 days.",
							)}
						</li>
						<li>
							•{" "}
							{t(
								"settings.audioConsent3",
								"Only you and your emergency contacts can access them.",
							)}
						</li>
						<li>
							•{" "}
							{t(
								"settings.audioConsent4",
						...
						<li>
							•{" "}
							{t(
								"settings.audioConsent5",
								"This feature processes health data (GDPR Art. 9).",
							)}
						</li>
					</ul>

...
export default function App() {
	...
	// ─── AI RISK DETECTION ───
	const { riskEvent, dismissRisk, acknowledgeAsSOS, currentSpeedMs } =
		useRiskDetection(userId);
	...
	useEffect(() => {
		const handleInactivityUpdate = () => {
			setIsInactivityActive(
				localStorage.getItem("eris_inactivity_sos_enabled") !== "false",
			);
		};
		window.addEventListener(
			"eris-inactivity-preference-changed",
			handleInactivityUpdate,
		);
		return () =>
	...
	const { resetTimer: resetInactivityTimer } = useInactivityMonitoring(
			...
			if (userPosition.lat !== 0 && userPosition.lng !== 0) {
				try {
					...
					const emergencyMessage = isTimeout
						? t(
								"alert.autoFallNote",
								"AUTOMATIC FALL DETECTED: Fall detected and user unresponsive.",
							)
						: t(
								"alert.manualFallNote",
								"FALL CONFIRMED: User manually confirmed the fall.",
							);
			...
			if (userPosition.lat !== 0 && userPosition.lng !== 0) {
				try {
					...
					const emergencyMessage = isTimeout
						? t(
								"alert.autoCrashNote",
								"AUTOMATIC CRASH DETECTED: Severe vehicle crash detected and user unresponsive.",
							)
						: t(
								"alert.manualCrashNote",
								"CRASH CONFIRMED: User manually confirmed the vehicle crash.",
							);
	...
	const [visualTheme, setVisualTheme] = useState(
		localStorage.getItem("eris_theme") || "dark",
	);
	...
	useEffect(() => {
		const handlePreferenceUpdate = () => {
			const freshValue =
				localStorage.getItem("eris_shake_sos_enabled") !== "false";
			setIsShakeActive(freshValue);
		};

		window.addEventListener(
			"eris-shake-preference-changed",
	...
	useEffect(() => {
		...
		const meshListenerPromise = CapacitorErisSosmap.addListener(
		...
		return () => {
			...
			meshListenerPromise
				.then((listener: PluginListenerHandle) => listener.remove())
				.catch(() => {});
			stopListening();
		};
	...
	return (
		<div className="flex flex-col h-screen w-full bg-eris-bg text-eris-text overflow-hidden font-sans">
			...
			<header className="flex justify-between items-center px-5 py-3 bg-eris-bg/95 backdrop-blur-md border-b border-eris-border/50 z-[1000] relative">
				...
				<h1 className="flex-1 text-center text-eris-text text-lg font-bold tracking-wide">
					ERIS Safety
				</h1>
			...
			<main className="flex-1 relative overflow-hidden">
				...
				<div
					className={`absolute inset-0 ${activeTab === "MAP" ? "z-10" : "hidden"}`}
				>
					<MapScreen
			...
			{!showFallModal && !showCrashModal && showInactivityModal && (
				<InactivityModal
					...
					onConfirmSOS={handleInactivitySOS}
				/>
			)}
					...
					return (
						<button
							...
							className={`flex flex-col items-center justify-center w-16 gap-1 transition-all ${
								isActive
									? "text-eris-primary"
									: "text-eris-text-subtle hover:text-eris-text-muted"
							}`}
						>
							<div
